...
| sourceMapReference | `boolean`                                                                           | optional | Determines whether CLI should automatically emit sourcemap references. By default it will store a `Sourcemap` header with minified files so that sourcemaps are located automatically, without the need for `sourceMappingURL` if the tool can detect a link. Defaults to `true`.                                                                                               |
